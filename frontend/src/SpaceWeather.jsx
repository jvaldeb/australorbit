    ...
    <div style={{borderRadius:16,padding:"20px 22px",background:"rgba(255,255,255,0.018)",border:"1px solid rgba(255,255,255,0.06)",backdropFilter:"blur(10px)"}}>
      ...
      <div style={{display:"flex",alignItems:"flex-end",gap:2,height:80}}>
        {last24.map((d, i) => {
          ...
          const h     = Math.max(2, (v / maxKp) * 80);
          const color = v >= 7 ? "#f87171" : v >= 5 ? "#fbbf24" : v >= 3 ? "#57C7FF" : "#334155";
          return (
            <div key={i} title={`Kp ${v.toFixed(1)}`} style={{flex:1,height:`${h}px`,borderRadius:"2px 2px 0 0",background:color,opacity:0.8,transition:"height 0.3s",minWidth:0}}/>
          );
        })}
      </div>
...
export default function SpaceWeather() {
  ...
  const fetchData = () => {
    ...
    fetch("https://services.swpc.noaa.gov/json/planetary_k_index_1m.json")
      ...
      .then(d => {
        if (d.length) {
          setKpHistory(d.slice(-48));
          setKp(parseFloat(d[d.length - 1].kp_index));
        ...
        <div style={{maxWidth:940,margin:"0 auto"}}>
          ...
          <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 0 16px",borderBottom:"1px solid rgba(255,255,255,0.045)",animation:"fadeIn 0.6s ease both"}}>
            ...
            <div style={{display:"flex",gap:6}}>
              {[["/ Inicio","/"],["🚀 Lanzamientos","/lanzamientos"],["🌌 Clima espacial","/espacio"]].map(([l,h])=>(
                <a key={h} href={h} style={{fontSize:10,fontFamily:"'IBM Plex Mono',monospace",color:window.location.pathname===h?"#6EE7B7":"#334155",padding:"5px 10px",borderRadius:7,background:window.location.pathname===h?"rgba(110,231,183,0.1)":"transparent",border:`1px solid ${window.location.pathname===h?"rgba(110,231,183,0.25)":"transparent"}`,textDecoration:"none",letterSpacing:"0.06em",transition:"all 0.2s"}}>{l}</a>
              ))}
            </div>
          </nav>
          ...
          {loading && (
            <div style={{padding:60,textAlign:"center"}}>
              ...
              <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:"#57C7FF",letterSpacing:"0.16em"}}>Conectando con NOAA...</div>
            </div>
          )}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}} className="sw-grid">
                ...
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:11,color:"#64748b",fontFamily:"'IBM Plex Mono',monospace"}}>
                      Kp {currentKp.toFixed(1)} · Zona visible estimada: {auroraZone.lat} ({auroraZone.place})
                    </div>
                  </div>
                </div>
              ...
              {wind && (
                <div className="stat-card">
                  ...
                  {wind.bz_gsm < -10 && (
                    <div style={{marginTop:10,padding:"7px 11px",borderRadius:8,background:"#f8741610",border:"1px solid #f8741630",fontSize:10,color:"#fb923c",fontFamily:"'IBM Plex Mono',monospace"}}>
                      ⚠️ Bz negativo intenso — mayor probabilidad de aurora
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="stat-card">
                ...
                <div style={{display:"flex",flexDirection:"column",gap:7}}>
                  {[
                    ...
                    { g:"G2", kpMin:6, label:"Moderada", desc:"Alertas de voltaje en redes", color:"#fb923c" },
                    { g:"G3", kpMin:7, label:"Fuerte", desc:"Posible aurora en latitudes medias", color:"#f87171" },
                    ...
                    <div key={row.g} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 10px",borderRadius:8,background:currentKp>=row.kpMin?`${row.color}10`:"rgba(255,255,255,0.02)",border:`1px solid ${currentKp>=row.kpMin?row.color+"30":"rgba(255,255,255,0.04)"}`,transition:"all 0.5s"}}>
                      ...
                      <div style={{flex:1}}>
                        <div style={{fontSize:10,color:currentKp>=row.kpMin?"#E0E8F0":"#334155",fontWeight:500,transition:"color 0.5s"}}>{row.label}</div>
                        <div style={{fontSize:8.5,color:"#1E3A50",fontFamily:"'IBM Plex Mono',monospace"}}>{row.desc}</div>
                      </div>
              ...
              {alerts.length > 0 && (
                <div style={{gridColumn:"1 / -1",borderRadius:16,padding:"20px 22px",background:"rgba(255,255,255,0.018)",border:"1px solid rgba(255,255,255,0.06)",backdropFilter:"blur(10px)"}}>
                  ...
                  {alerts.map((a, i) => (
                    <div key={i} style={{padding:"10px 14px",borderRadius:9,background:"rgba(3,8,20,0.4)",border:"1px solid rgba(255,255,255,0.05)",marginBottom:7,fontSize:11,color:"#64748b",lineHeight:1.6,fontFamily:"monospace"}}>
                      {a.message?.split("\n").slice(0,3).join(" · ") || "—"}
                    </div>
                  ))}
                </div>
          ...
          <div style={{padding:"14px 0 22px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
            ...
            <div style={{fontSize:8.5,color:"#0C2040",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.08em"}}>Datos: NOAA Space Weather Prediction Center</div>
          </div>
        </div>
      </div>
