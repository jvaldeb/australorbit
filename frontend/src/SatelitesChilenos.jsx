...
export default function SatelitesChilenos() {
  usePageMeta({
    title: "Satélites chilenos en órbita — historia completa desde 1995",
    description: "Todos los satélites chilenos: FASat-Alfa, Bravo, Charlie, SUCHAI-1, PlantSat, SUCHAI-2, SUCHAI-3, FASat-Delta y LEMU NGE. Historia, misión y posición en vivo.",
  ...
  useEffect(() => {
    const active = CHILEAN_SATS.filter(s => s.norad && s.status === "EN ÓRBITA");
    active.forEach(sat => {
      const go = () => {
    ...
    <>
      ...
      <div style={{position:"fixed",inset:0,zIndex:0,overflow:"hidden",pointerEvents:"none"}}>
        ...
        <div style={{position:"absolute",inset:0,animation:"earthFadeIn 2.5s ease both"}}>
          <img src="/earth-bg.png" alt="" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:"90vmin",height:"90vmin",objectFit:"contain",opacity:0.08,animation:"earthDrift 55s ease-in-out infinite",filter:"saturate(0.5) brightness(0.7)"}}/>
        </div>
        <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse 120% 90% at 50% 50%, transparent 0%, rgba(0,0,0,0.75) 60%, rgba(0,0,0,0.96) 100%)"}}/>
        <svg style={{position:"absolute",inset:0,width:"100%",height:"100%",opacity:0.4}}>
          {Array.from({length:80},(_,i)=>(
            <circle key={i} cx={`${(i*137.5)%100}%`} cy={`${(i*97.3)%100}%`} r={i%5===0?1.2:0.6} fill="white" opacity={0.2+((i*0.13)%0.4)}/>
          ))}
        </svg>
        ...
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          {menuOpen && (
            <div style={{position:"fixed",top:0,left:0,right:0,zIndex:150,background:"rgba(0,0,0,0.97)",borderBottom:"1px solid rgba(255,255,255,0.08)",backdropFilter:"blur(30px)",padding:"80px 24px 24px",display:"flex",flexDirection:"column",gap:4,animation:"fadeUp 0.2s ease both"}}>
              {LINKS.map(([label,href])=>(
                <a key={href} href={href} style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:700,color:href==="/satelites-chilenos"?"#fff":"rgba(255,255,255,0.5)",textDecoration:"none",padding:"14px 0",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>{label}</a>
              ))}
            </div>
          )}
          ...
          <nav style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 0 14px",borderBottom:"1px solid rgba(255,255,255,0.07)",animation:"fadeIn 0.7s ease both",gap:16,position:"relative",zIndex:160}}>
            <a href="/" style={{display:"flex",alignItems:"center",gap:12,textDecoration:"none",flexShrink:0}}>
              {!logoError
                ?<img src="/logo.png" alt="Austral Orbit" onError={()=>setLogoError(true)} style={{height:44,width:"auto",objectFit:"contain",filter:"drop-shadow(0 2px 16px rgba(10,28,80,0.6)) brightness(1.08)"}}/>
          ...
          <div style={{padding:"56px 0 48px",animation:"fadeUp 0.9s ease both"}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:8,padding:"5px 14px",borderRadius:99,...glass({}),marginBottom:24}}>
              ...
              <span style={{fontSize:8.5,fontFamily:"'IBM Plex Mono',monospace",color:"#C47B48",letterSpacing:"0.2em",textTransform:"uppercase"}}>Programa espacial chileno · 1995–2024</span>
            </div>
            ...
            <p style={{fontSize:15,color:"rgba(255,255,255,0.35)",lineHeight:1.8,fontWeight:300,maxWidth:560,marginBottom:32}}>
              30 años de historia espacial. Desde el fallido FASat-Alfa en 1995 hasta el LEMU NGE lanzado por SpaceX en 2024 — la historia completa de Chile conquistando el espacio.
            </p>
            <div style={{display:"flex",gap:24,flexWrap:"wrap"}}>
              {[["9","satélites lanzados"],["1995","primer intento"],["5","actualmente en órbita"],["30","años de historia"]].map(([n,l])=>(
                <div key={l}>
          ...
          {/* Cards */}
          <div className="cards-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,paddingBottom:60}}>
            {CHILEAN_SATS.map((sat,i)=>(
              <div key={sat.id} style={{animation:`fadeUp 0.6s ease ${i*0.08}s both`}}>
          ...
          <div style={{padding:"16px 0 24px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
            ...
            <div style={{fontSize:8,color:"rgba(255,255,255,0.12)",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:"0.08em"}}>Datos verificados: NORAD · CelesTrak · Wikipedia · Defensa.cl</div>
          </div>
        </div>
      </div>
    </>
