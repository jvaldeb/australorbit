    ...
    <div style={{borderRadius:16,border:`1px solid ${isNext?"rgba(87,199,255,0.28)":"rgba(255,255,255,0.07)"}`,background:"rgba(255,255,255,0.014)",backdropFilter:"blur(10px)",marginBottom:12,overflow:"hidden",transition:"all 0.2s",boxShadow:isNext?"0 0 30px rgba(87,199,255,0.06)":"none"}}>
      {img && (
        <div style={{position:"relative",height:isNext?200:140,overflow:"hidden",cursor:"pointer"}} onClick={()=>setOpen(o=>!o)}>
          <img src={img} alt={launch.name} style={{width:"100%",height:"100%",objectFit:"cover",opacity:0.75,display:"block"}} onError={()=>setImgErr(true)}/>
      ...
      <div style={{padding:"14px 18px",cursor:"pointer"}} onClick={()=>setOpen(o=>!o)}>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
          ...
          {!img && <div style={{textAlign:"right",flexShrink:0}}>{cd?<div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:isNext?20:15,fontWeight:600,color:isNext?"#57C7FF":"#F0F4F8",letterSpacing:"0.04em"}}>{cd}</div>:<div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:12,color:"#334155"}}>Lanzado</div>}<div style={{fontSize:8.5,color:"#334155",fontFamily:"'IBM Plex Mono',monospace",marginTop:2}}>{fmtDate(launch.net)}</div></div>}
          <span style={{color:"#1E3A50",fontSize:11,flexShrink:0,paddingTop:2}}>{open?"▲":"▼"}</span>
        </div>
        <div style={{display:"flex",gap:14,marginTop:10,flexWrap:"wrap"}}>
          {[["🚀",rocket],["📍",pad_name],["🌍",location]].map(([icon,v])=>(
            <div key={icon} style={{display:"flex",alignItems:"center",gap:4,fontSize:10,color:"#64748b",fontFamily:"'IBM Plex Mono',monospace"}}><span style={{fontSize:11}}>{icon}</span><span style={{color:"#8090A0"}}>{v}</span></div>
          ))}
        </div>
      </div>
      {open && (
        <div style={{padding:"14px 18px",borderTop:"1px solid rgba(255,255,255,0.05)",animation:"fadeUp 0.2s ease both"}}>
          {mdesc && <><div style={{fontSize:8.5,fontFamily:"'IBM Plex Mono',monospace",color:"#1E3A50",letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:8}}>Descripción de la misión</div><p style={{fontSize:12,color:"#64748b",lineHeight:1.72,fontWeight:300,marginBottom:12}}>{mdesc}</p></>}
    ...
    <>
      ...
      <div className="page-pad" style={{position:"relative",zIndex:1,padding:"0 22px",minHeight:"100vh"}}>
        <div style={{maxWidth:900,margin:"0 auto"}}>
          ...
          <Nav logoError={logoError} setLogoError={setLogoError} menuOpen={menuOpen} setMenuOpen={setMenuOpen} activePath="/lanzamientos"/>
          ...
          <div style={{padding:"24px 0 48px"}}>
            {loading && <div style={{padding:60,textAlign:"center"}}><div style={{fontSize:28,marginBottom:14,display:"inline-block",animation:"livePulse 1.5s infinite"}}>🚀</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:"#57C7FF",letterSpacing:"0.16em"}}>Cargando lanzamientos...</div></div>}
            ...
            {!loading && !error && launches.map((l,i) => <LaunchCard key={l.id} launch={l} isNext={i===0}/>)}
          </div>
          ...
          <div style={{padding:"14px 0 22px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
            ...
            <div style={{fontSize:8.5,color:"#0C2040",fontFamily:"'IBM Plex Mono',monospace"}}>Datos: The Space Devs · Launch Library 2</div>
          </div>
        </div>
      </div>
    </>
